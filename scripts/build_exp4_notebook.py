#!/usr/bin/env python3
"""Build notebooks/exp4_stageA_extraction_colab.ipynb.

Experiment 4 — Stage A concept extraction (LR only):
  Stage A:  X -> B-hat   (ridge linear extractor over the 1,408 shared ideas)
  Stage B:  X + B-hat -> Y   (logistic probe, same config as exp2/exp3)

Measures how much of the +9.1pt shared-B lift a *linear, label-blind*
extractor recovers. Ceiling = golden shared-B (exp3 C: 0.747). Floor = X-only
(exp3 A: 0.656). If derived B-hat lands near the floor, linear extraction is
too weak to exploit the bottleneck (the QSBC Phase-3 thesis).

Committed inputs:
  results/exp1/samples_2858.csv
  results/exp3/sample_ideas_shared.csv
"""
...
with open("notebooks/exp4_stageA_extraction_colab.ipynb", "w") as fh:
    json.dump(notebook, fh, indent=1)
print("wrote notebooks/exp4_stageA_extraction_colab.ipynb with", len(cells), "cells")