#!/usr/bin/env python3
"""Build notebooks/exp3_lr_singleton_ablation_colab.ipynb.

Experiment 3 (LR only) — singleton ablation on golden B:
  A. X -> Y                              (control)
  B. X + C (full golden B, 8,706 ideas)  (exp2 result)
  C. X + C (no singletons, 1,408 ideas)  (this experiment)

Tests the hypothesis: singleton ideas (support == 1) carry no classifiable
signal, so dropping them should barely move the needle. Committed inputs:
  results/exp1/samples_2858.csv         (sentence_id, doc_key, sentence, label)
  results/exp1/sample_ideas.csv         (full golden B mapping)
  results/exp3/sample_ideas_shared.csv  (golden B restricted to support >= 2)
"""
...
with open("notebooks/exp3_lr_singleton_ablation_colab.ipynb", "w") as fh:
    json.dump(notebook, fh, indent=1)
print("wrote notebooks/exp3_lr_singleton_ablation_colab.ipynb with", len(cells), "cells")