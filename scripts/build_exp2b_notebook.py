#!/usr/bin/env python3
"""Build notebooks/exp2b_bert_x_vs_xc_colab.ipynb.

Experiment 2 — BERT leg of the "one thing" test:
  A. BERT X -> Y                    (control)
  B. BERT X + C -> Y                (experiment: idea-bottleneck supplement)

Identical to exp2 but with a fine-tuned BERT backbone instead of TF-IDF+LR.
Same 60/40 document-level split, same seed, same epochs/lr/batch.

For B, C (multi-hot over the 8,706-idea space) is concatenated with BERT's
[CLS] pooled embedding before a linear classification head, so the C-block
attribution stays linear/exact. Committed inputs:
  results/exp1/samples_2858.csv
  results/exp1/sample_ideas.csv
"""
...
cells.append({
    ...
    "source": [
        ...
        "for p in modelA.bert.parameters():\n",
        "    p.requires_grad = False   # frozen backbone: train only the head (clean probe)\n",
...
cells.append({
    ...
    "source": [
        ...
        "for p in modelB.bert.parameters():\n",
        "    p.requires_grad = False   # frozen backbone: train only the head (clean probe)\n",
...
with open("notebooks/exp2b_bert_x_vs_xc_colab.ipynb", "w") as fh:
    json.dump(notebook, fh, indent=1)
print("wrote notebooks/exp2b_bert_x_vs_xc_colab.ipynb with", len(cells), "cells")